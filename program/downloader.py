...
@Client.on_message(command(["صوت", "song", "تحميل صوت", "تنزيل صوت", f"song@{bn}"]) & ~filters.edited)
def song(_, message):
...
async def vsong(client, message):
    ...
    try:
        ...
        print(e)
    try:
        msg = await message.reply("📥 **يتم تحح ــــميل آآلفيديؤؤ...**")
    ...
    await msg.edit("📤 **ج ــآآرري تحح ــــميل آآلفيديؤؤ...**")
...
async def lyrics(_, message):
    try:
        ...
        resp = requests.get(
            f"https://api.medooo.ml/leomedo/voiceRecognise?token=bot_token&chat_id=chat_id&file_id=idd&msg_id={query}"
        ).json()
        result = f"{resp['data']}"
