...
async def update_admin(client, message):
    ...
    await message.reply_text(
        "**البوت اشتغل لو واجهتك اي مشكلة تواصل معانا ياغالي تحياتي لكم**"
    )
...
@Client.on_message(
    command(["pause", "قف", "ؤقتا", f"pause@{BOT_USERNAME}", "vpause"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["volume", "بط", f"volume@{BOT_USERNAME}", "vol"]) & other_filters
)
@authorized_users_only
